...
class AllC:
	...
	def merge(self):
		...
		for chrom in sorted(self.chroms):
			fi=open(os.path.join(self.outdir,chrom+'.bed'),'r')
			line=fi.readline()
			while line:
				values=line.strip().split('\t')
				fo.write('\t'.join(values[:4]+[str(ID),values[-1]])+'\n')
				line=fi.readline()
				ID+=1
			fi.close()
			os.remove(os.path.join(self.outdir,chrom+'.bed'))
		fo.close()

	def bgzip(self):
		if os.path.exists(f"{self.out}.gz"):
			return None
		print(f"Compressing {self.out}...")
		cmd=f"bgzip {self.out} && tabix -b 2 -e 3 -0 {self.out}.gz"
		os.system(cmd)
